...
1. Python 3 will need to be installed prior to setting up MAT, preferably any python
    greater than 3.7. Python 3 can be downloaded from python.org. The x86-64 executable
    installer is reccommended, as the default install configuration will set python to
    open .py files by double clicking. If it is installed correctly opening a cmd
    or powershell window and typing `py --version` will print the installed python version.

2.  Install MAT by opening a cmd or powershell window and running `py -m install hundlab-MAT`,
     This should install MAT and all of its dependancies. 
...
3.  Once MAT has been installed it can be run via cmd, powershell or the start menu. To run
     ...
     Desktop right-click -> new -> new shortcut and paste the path when it asked for a path.
     ...
     is a Scripts directory and the `MacrophageAnalysisToolkit.py` will be in there, and
     a shortcut can be made to it directly and placed on the desktop.
...
1. Python 3 will need to be installed prior to setting up MAT. Python 3 can be 
    ...
    the command is python3) should start a python prompt. It may also be necessary
